...
static enum { method_GET=0, method_POST=1, method_PUT=2 } http_method = method_GET; // 0 = get, 1 = post, 2 = put
static void (*client_browser_callback)(uint8_t,uint16_t,uint16_t); // the fields are: uint8_t webstatuscode,uint16_t datapos,uint16_t len; webstatuscode==0 means 2xx was the answer from the web server; datapos is start of http data and len the the length of that data
...
uint16_t www_client_internal_datafill_callback(uint8_t fd){
        ...
        if (fd==www_fd){
				...
				else if( http_method == method_PUT )
				{
					// PUT
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data){
        uint8_t web_statuscode=1; // tcp status is OK but we need to check http layer too
        if (fd!=www_fd){
        ...
        if (statuscode==0 && len_of_data>12){
                ...
                web_statuscode=0; // failure
                if (client_browser_callback){
                        if (isblank(bufptr[datapos+8]) && isdigit(bufptr[datapos+9])&& isdigit(bufptr[datapos+10])){ // e.g 200 OK, we put the first digit into the web_statuscode:
                                web_statuscode=bufptr[datapos+9]-'0';
                        }
...
// client_browser_url(PSTR("/cgi-bin/checkip"),NULL,PSTR("tuxgraphics.org"),&browserresult,other_side_ip,gwmac);
...
// webstatuscode is otherwise the first digit of the http status code (e.g webstatuscode=2 for 200 OK);
// You can read the full status code as string if you look at buf[datapos+9]. buf[datapos+9] would be the character of the first digit and buf[datapos+11] the last digit.
// For possible status codes look at http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html
// Basically 2xx (webstatuscode=2) is success and any 5xx, 4xx (webstatuscode>=4) is a failure.
// The string buffers to which urlbuf_varpart and hoststr are pointing
// must not be changed until the callback is executed.
//
void client_browse_url(const prog_char *urlbuf,const char *urlbuf_varpart,const char *hoststr,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac)
{
...
void client_http_post(const prog_char *urlbuf, const char *urlbuf_varpart,const char *hoststr, const prog_char *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac)
{
        client_urlbuf=urlbuf;
        client_hoststr=hoststr;
        client_urlbuf_var=urlbuf_varpart;
...
// when the post operation was really done (e.g when callback was executed).
// postval must be urlencoded.
void client_http_put(const prog_char *urlbuf, const char *urlbuf_varpart, const char *hoststr, const prog_char *additionalheaderline,char *postval,void (*callback)(uint8_t,uint16_t,uint16_t),uint8_t *dstip,uint8_t *dstmac)
{
	client_urlbuf = urlbuf;
	client_hoststr = hoststr;
	client_urlbuf_var = urlbuf_varpart;
